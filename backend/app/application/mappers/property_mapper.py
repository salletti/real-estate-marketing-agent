...
def _parse_datetime(value: Optional[str]) -> Optional[datetime]:
    # Accepte les formats ISO 8601 ; retourne None si absent ou invalide
    if not value:
        return None
    try:
...
def _extract_type(data: Dict[str, Any]) -> Dict[str, Any]:
    # Champs de classification du bien
    return {
...
def _extract_living_area(data: Dict[str, Any]) -> Dict[str, Any]:
    # Surfaces et composition des pièces
...
def _extract_price(data: Dict[str, Any]) -> Dict[str, Any]:
    # Prix de mandat, charges et taxes
    return {
...
def _extract_environment(data: Dict[str, Any]) -> Dict[str, Any]:
    # Équipements collectifs et stationnement
...
def _extract_amenities(data: Dict[str, Any]) -> List[str]:
    # Fusion comfort + security, doublons supprimés, ordre stable
